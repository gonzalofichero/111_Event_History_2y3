...
##### Q2. Provide a table with estimated coefficients and 95% confidence interval. Are the three group statistically different? #####
...
# Over the possible intervals (quarters)
x <- seq(1.5,22.5,by=3)
...
# True baseline hazard (density over survival)
true.haz <- dweibull(x, shape=a, scale=b) / (pweibull(x, shape=a, scale=b, lower.tail=F))

# All together
wei.hazard <- data.frame(Time = x,
                         Logistic = h1,
                         Cloglog = h2,
                         True = true.haz)
...
# Plotting
wei.hazard %>% 
  pivot_longer(Logistic:True, names_to = "Group", values_to = "Hazard", values_drop_na = F) %>% 
  ggplot(aes(x=Time, y=Hazard, color=Group, shape= Group)) + geom_point(size=3, alpha=0.5) + 
  labs(#title = "True and estimated baseline hazard from a Discrete Time Model \n with two different specifications",
    x = "Time", 
    y = "Discrete hazard") +
